...
export function defaultPostSuccess(payload, response) {}

export const createAsyncCallSaga = (
    callAction,
    successAction,
    failureAction,
    asynFunction,
    postSagaSuccess = defaultPostSuccess
) => {
    function* makeCall({ payload }) {
        try {
            ...
            if (postSagaSuccess) {
                yield call(postSagaSuccess, payload, response);
            }
        } catch (e) {
