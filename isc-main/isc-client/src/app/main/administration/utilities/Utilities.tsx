...
const entityFields: IEntityField[] = [
    ...
    { name: 'status', type: 'enum', valueOptions: ['ACTIVATED', 'DEACTIVATED'], defaultValue: 'ACTIVATED' },
    {
        ...
        defaultValue: 'VENTILATION_SHUTOFF',
    },
    {
        name: 'school',
        inputLabel: 'School',
...
export default function EnhancedTable(): JSX.Element {
    ...
    const queryAction = (context: IscTableContext): Promise<SearchResult> => {
        ...
        return queryService.queryUtilitiesWithFilter(
            filter,
            {
    ...
    const createAction = (data: any): Promise<any> => {
        return queryService.createUtility(data).then((payload: any) => {
            dispatch(Actions.addUtility(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const updateAction = (data: any): Promise<any> => {
        return queryService.updateUtility(data).then((payload: any) => {
            dispatch(Actions.updateUtilitySuccess(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const deleteAction = (data: any): Promise<any> => {
        return queryService.deleteUtility(data).then((payload: any) => {
            dispatch(Actions.deleteUtilitySuccess(payload));
            dispatch(Actions.loadRegionTree(true));
            return payload;
        });
    };
