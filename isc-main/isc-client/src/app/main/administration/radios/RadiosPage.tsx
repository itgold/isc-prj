...
export default function EnhancedTable(): JSX.Element {
    ...
    const queryAction = (context: IscTableContext): Promise<SearchResult> => {
        ...
        return deviceRadioService.queryRadiosWithFilter(
            filter,
            {
    ...
    const createAction = (data: any): Promise<any> => {
        return deviceRadioService.createRadio(data).then((payload: any) => {
            dispatch(Actions.addRadio(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const updateAction = (data: any): Promise<any> => {
        return deviceRadioService.updateRadio(data).then((payload: any) => {
            dispatch(Actions.updateRadioSuccess(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const deleteAction = (data: any): Promise<any> => {
        return deviceRadioService.deleteRadio(data).then((payload: any) => {
            dispatch(Actions.deleteRadioSuccess(payload));
            dispatch(Actions.loadRegionTree(true));
            return payload;
        });
    };
