...
export default function EnhancedTable(): JSX.Element {
    ...
    const queryAction = (context: IscTableContext): Promise<SearchResult> => {
        ...
        return queryService.querySafetiesWithFilter(
            filter,
            {
    ...
    const createAction = (data: any): Promise<any> => {
        return queryService.createSafety(data).then((payload: any) => {
            dispatch(Actions.addSafety(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const updateAction = (data: any): Promise<any> => {
        return queryService.updateSafety(data).then((payload: any) => {
            dispatch(Actions.updateSafetySuccess(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const deleteAction = (data: any): Promise<any> => {
        return queryService.deleteSafety(data).then((payload: any) => {
            dispatch(Actions.deleteSafetySuccess(payload));
            dispatch(Actions.loadRegionTree(true));
            return payload;
        });
    };
