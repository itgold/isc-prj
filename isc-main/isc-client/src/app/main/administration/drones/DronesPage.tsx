...
const entityFields: IEntityField[] = [
    ...
    { name: 'status', type: 'enum', valueOptions: ['ACTIVATED', 'DEACTIVATED'], defaultValue: 'ACTIVATED' },
    {
        name: 'type',
        type: 'enum',
        valueOptions: ['TETHERED'],
        defaultValue: 'TETHERED',
    },
    {
        name: 'school',
        inputLabel: 'School',
...
export default function EnhancedTable(): JSX.Element {
    ...
    const queryAction = (context: IscTableContext): Promise<SearchResult> => {
        ...
        return queryService.queryDronesWithFilter(
            filter,
            {
    ...
    const createAction = (data: any): Promise<any> => {
        return queryService.createDrone(data).then((payload: any) => {
            dispatch(Actions.addDrone(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const updateAction = (data: any): Promise<any> => {
        return queryService.updateDrone(data).then((payload: any) => {
            dispatch(Actions.updateDroneSuccess(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const deleteAction = (data: any): Promise<any> => {
        return queryService.deleteDrone(data).then((payload: any) => {
            dispatch(Actions.deleteDroneSuccess(payload));
            dispatch(Actions.loadRegionTree(true));
            return payload;
        });
    };
