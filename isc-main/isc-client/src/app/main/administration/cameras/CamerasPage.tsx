...
export default function EnhancedTable(): JSX.Element {
    ...
    const queryAction = (context: IscTableContext): Promise<SearchResult> => {
        ...
        return queryService.queryCamerasWithFilter(
            filter,
            {
    ...
    const createAction = (data: any): Promise<any> => {
        return queryService.createCamera(data).then((payload: any) => {
            dispatch(Actions.addCamera(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const updateAction = (data: any): Promise<any> => {
        return queryService.updateCamera(data).then((payload: any) => {
            dispatch(Actions.updateCameraSuccess(payload));
            dispatch(Actions.loadRegionTree(true));
        });
    };
    const deleteAction = (data: any): Promise<any> => {
        return queryService.deleteCamera(data).then((payload: any) => {
            dispatch(Actions.deleteCameraSuccess(payload));
            dispatch(Actions.loadRegionTree(true));
            return payload;
        });
    };
