...
@Slf4j
public class IncrementalUpdatesSearchRepositoryCustomImpl extends BaseCustomSearchRepository implements IncrementalUpdatesSearchRepositoryCustom {
    ...
    public Page<ApplicationEventVo> findAllWithFilter(QueryFilterDto filter, Pageable pageable) throws IOException {
        ...
        BoolQueryBuilder queryBuilder = QueryBuilders.boolQuery()
                .must(QueryBuilders.termQuery("_index", ApplicationEventVo.INDEX))
                .must(QueryBuilders.termQuery("eventClass", IncrementUpdateEventVo.TYPE));
        ...
        if (failures != null && failures.length > 0) {
            String errors = Arrays.stream(failures).map(ShardOperationFailedException::reason).collect(Collectors.joining("\n"));
            ...
            long totalHits = hits != null && hits.getTotalHits() != null ? hits.getTotalHits().value : 0L;
    ...
    public List<StateEventVo> queryDeviceMostRecentEvents(List<Class<?>> eventClasses, List<String> deviceIds) throws IOException {
        ...
        if (searchHits != null && searchHits.length > 0) {
            for (SearchHit hit : hits.getHits()) {
                ...
                SearchHits innerHits = hit.getInnerHits().get("some_hints_name");
